...
static void hippolink_fill_radio_rssi_report(uint8_t node_id,
                                             uint8_t remote_id);
...
static __pdata uint8_t _seqnum = 0;
...
bool hippolink_rssi_report(uint8_t node_id, uint8_t node_count) {
    ...
    full_packet_size = (sizeof(struct hippolink_radio_rssi_report_s) +
                        HEADER_LENGTH + CRC_LENGTH + COBS_OVERHEAD);
    if (serial_write_space() < full_packet_size) return false;
    _node_index++;
    if (_node_index == node_id) _node_index++;
    if (_node_index >= node_count) _node_index = 0;
...
static void hippolink_fill_radio_rssi_report(uint8_t node_id,
    ...
    header = (struct hippolink_header_s *)&_pbuf[COBS_OFFSET];
    payload = (struct hippolink_radio_rssi_report_s
                   *)&_pbuf[COBS_OFFSET + HEADER_LENGTH];
...
static void hippolink_crc(register uint8_t crc_extra) {
    ...
    _pbuf[i - 1] = (uint8_t)(sum & 0xFF);
    _pbuf[i] = (uint8_t)(sum >> 8);
}

static void cobs_enocde(register uint8_t hippolink_len) {
    __data uint8_t i;
    __data uint8_t offset = 0;
    _pbuf[0] = 0;
    _pbuf[hippolink_len + 1] = 0;
    for (i = 1; i <= hippolink_len + 1; i++) {
        offset++;
        if (!_pbuf[i]) {
            _pbuf[i - offset] = offset;
            offset = 0;
        }
    }
}